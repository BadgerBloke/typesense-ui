        ...
        <div className="flex w-full flex-wrap items-center gap-2">
            {itemsArray?.map((item, index) =>
                index === itemsArray.length - 1 ? (
                    <Typography variant="muted" key={slugify(item.label)}>
                        <span className="flex cursor-not-allowed items-center gap-1">
                            {index === 0 && <IconLayoutDashboard className="h-4 w-4" />} {item.label.toSentenceCase()}
                        </span>
                    </Typography>
                    ...
                    <Typography variant="small" key={slugify(item.label)}>
